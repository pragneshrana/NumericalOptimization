...
In city planning, the issue is to find out potential cites to plant the towers for given the population and budget constrain. To solve the problem, divide the population into small region based on maximum capacity to plant the tower sites. Each site location may also cover other specified number of locations. Out of all possible cell tower cites, find out the optimal locations which can serve maximum  population keeping control over budget.




\section{Approach to tackle the problem (In short)}
In Fundamental of cellular network\cite{david} it was mentioned that cellular network is 
\begin{itemize}
	...
	\item After generating population, the whole population is divided into given number small regions(clusters) using k-means algorithm.
